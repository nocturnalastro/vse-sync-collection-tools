...
func (dpllInfo *DevNetlinkDPLLInfo) GetAnalyserFormat() ([]*callbacks.AnalyserFormatType, error) {
	var subType string
	if dpllInfo.PinType == OnePPSLabel {
		subType = "dpll"
	} else if dpllInfo.PinType == SMA1Label {
		subType = "dpll-sma1"
	} else {
		subType = "Unknown"
	}
		...
		[]fetcher.AddCommandArgs{
			{
				...
				Command: "/linux/tools/net/ynl/cli.py --spec /linux/Documentation/netlink/specs/dpll.yaml --dump device-get | python3 /root/custom_scripts/json_encoder.py",
				Trim:    true,
			},
			{
				Key: "dpll-netlink-offset",
				Command: fmt.Sprintf(
					"/linux/tools/net/ynl/cli.py --spec /linux/Documentation/netlink/specs/dpll.yaml --do pin-get --json %s | python3 /root/custom_scripts/json_encoder.py",
	...
	fetcherInst, err := fetcher.FetcherFactory(
		...
		[]fetcher.AddCommandArgs{
			...
			},
			{
				...
				Command: "/linux/tools/net/ynl/cli.py --spec /linux/Documentation/netlink/specs/dpll.yaml --dump pin-get | python3 /root/custom_scripts/json_encoder.py",
				Trim:    true,
			},
		},
	)
...
func postProcessDPLLNetlinkClockID(result map[string]string) (map[string]any, error) {
	...
	err = json.Unmarshal([]byte(result["dpll-netlink-pins"]), &entries)
	if err != nil {
		log.Errorf("Failed to unmarshal netlink output: %s", err.Error())
	}

	var PPS_PIN NetlinkPin
	var SMA1_PIN NetlinkPin
	...
	choosePPS := true
	for _, parentDev := range PPS_PIN.ParentDevices {
		if parentDev.State != "connected" {
			choosePPS = false
			break
		}
	}
	...
	chooseSMA1 := true
	for _, parentDev := range SMA1_PIN.ParentDevices {
		if parentDev.Direction != "input" || parentDev.State != "connected" {
			chooseSMA1 = false
			break
		}
	}
	...
	if choosePPS {
		processedResult["offsetPin"] = PPS_PIN.ID
		processedResult["pinType"] = OnePPSLabel
	} else if chooseSMA1 {
		processedResult["offsetPin"] = SMA1_PIN.ID
		processedResult["pinType"] = SMA1Label
	} else {
		return processedResult, fmt.Errorf("failed to determin correct offset pin")
	}
